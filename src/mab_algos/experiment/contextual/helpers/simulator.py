...
def run_contextual_bandit(context_dim, num_actions, dataset, algos):
  ...
  cmab = ContextualBandit_Simulator(context_dim, num_actions)
  ...
  for i in tqdm(range(num_contexts)):
    ...
    for j, a in enumerate(algos):
      a.update(context=context, action=actions[j], reward=rewards[j])
...
class ContextualBandit_Simulator(object):
